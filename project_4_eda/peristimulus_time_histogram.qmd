...
```{r}
### ODOR CELLS BASED ON MEAN STANDARDIZED FIRING RATE, NORMALIZED BY BASELINE FIRING RATE
...
for (rat in rats) {
  ...
  for (id in odor_ids_list[[1]]) {
    ...
    if (nrow(trial) > 0) {
      normalized_values <- normalize_time_window(trial, id, numbins) # Normalize to [o,1.25] interval, 1000 rows per trial
  ...
  interpolated_data <- data.frame(time = seq(0, 1, length.out = numbins), combined_trials)
...
```
```{r}
# SORTING ODOR CELLS BY GLMM FOR PLOTS SEPERATED BY ODOR
...
for (i in 1:5) {
  trials_list <- list()
  behav_data <- be_data_clean[[rats[i]]]
  timelag_data <- TimeLag_EnsembleMat[[rats[i]]]$test_data 
  odor_ids_list <- get_odor_ids_list(rats[i], behav_data)  # corrected argument to rats[i]
  combined_trials <- matrix(0, nrow = numbins, ncol = ncol(timelag_data) - 5) 
  
  rat_data <- TimeLag_EnsembleMat_predicted[[rats[i]]]$train_data
  null_period <- rat_data[rat_data$y == 6,]
  baseline <- get_baseline(null_period)
  # Loop through each trial_id in odor_ids_list and sum the normalized values
  for (id in odor_ids_list[[1]]) {
    trial <- timelag_data[timelag_data[["trial_id"]] == id,]
    if (nrow(trial) > 0) {
      normalized_values <- normalize_time_window(trial, id, numbins) # Normalize to [0,1.25] interval, 1000 rows per trial
      combined_trials <- combined_trials + normalized_values # Sum i^th row of each eligible trial
    }
  }
  # Create the new dataframe with interpolated values
  ...
  update_names <- function(names_list, suffix) {
    # Append the suffix to the names
    updated_names <- paste0(names_list, ".", suffix)
    return(updated_names)
  }
  
  # Update the column names of interpolated_data
  colnames(interpolated_data)[-1] <- update_names(colnames(timelag_data)[6:ncol(timelag_data)], i)
...
odor_data <- data.frame(time = seq(0, 1, length.out = nrow(data)))
for (i in 1:5) {
  odor_data[[paste0("Odor", i)]] <- scale(rowMeans(data[, top_neurons_per_odor[[paste0("Odor",i)]]$neuron, drop = FALSE]))
}

# Melt the dataframe for ggplot
melted_odor_data <- pivot_longer(odor_data, cols = -time, names_to = "Odor", values_to = "AverageActivity")

# Plot the data
ggplot(melted_odor_data, aes(x = time, y = AverageActivity, color = Odor)) +
  geom_line(alpha = 0.1) +  # Adding a base line with transparency for reference
  geom_smooth(se = FALSE, method = "loess", span = 0.2) +  # Adding smoothed lines
  labs(title = "Smoothed Standardized Average Activity of Neurons for Each Odor",
       x = "Percentage of Offline Period Elapsed",
       y = "Standardized Average Activity",
       color = "Odor") +
  theme_minimal()
```
